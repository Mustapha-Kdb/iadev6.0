...
def handle_home(request):
    return redirect('factory-batches')
...
def handle_profile(request):
    # Retrieve the username for the current user or set it as needed
...
class TasksListView(ListView):
    model = models.Task
    template_name = 'factory/tasks.html'  # <app>/<model>_<viewtype>.html
...
class BatchesListView(ListView):
    model = models.Batch
    template_name = 'factory/batches.html'  # <app>/<model>_<viewtype>.html
    ...
    def get_queryset(self):
        ...
        if 'users' in self.request.GET:
                qs = self.model.objects.filter(~Q(created_by=self.request.user)).order_by('-created_at')
        if 'currentusr' in self.request.GET:
                qs = self.model.objects.filter(created_by=self.request.user).order_by('-created_at')
        else:
        ...
        for batch in qs:
            ...
            all_status = []
            for task in tasks:
                all_status.append(task.state)
            # print(all_status)
            if len(all_status) > 0 :
                if 'pending' in all_status:
                    status = 'pending'
                elif 'failed' in all_status:
                    status = 'failed'
                else:
                    status = 'success'
            batch.status = status
